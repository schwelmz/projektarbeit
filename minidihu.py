...
if __name__ == '__main__':
    ...
    tajectory = strang_H_CN_FE(
        Vmhn0, # initial values
        rhs0=rhs_hodgkin_huxley,
        rhs1=(cn_sys_expl, cn_sys_impl),
        traj=True, # return full tajectory
        **time_discretization
    )

    # indices: trajectory[time_index, point alog x-axis, variable]
    out_stride = 1
    np.save("out.npy",tajectory[::out_stride, :, :])

    ######## plot results
    time_steps = tajectory.shape[0]
    step_stride = 400
    ...
    ax.plot(xs, tajectory[::-step_stride, :, 0].T)
    for idx,line in enumerate(ax.lines): line.set_color((cs[idx], 0.5, 0.5))
    ax.plot(xs, tajectory[0, :, 0], '--', color='black')
    ax.plot(xs, tajectory[-1, :, 0], color='black')

    ax = fig.add_subplot(412)
    ax.plot(xs, tajectory[::-step_stride, :, 1].T)
    for idx,line in enumerate(ax.lines): line.set_color((cs[idx], 0.5, 0.5))
    ax.plot(xs, tajectory[0, :, 1], '--', color='black')
    ax.plot(xs, tajectory[-1, :, 1], color='black')

    ax = fig.add_subplot(413)
    ax.plot(xs, tajectory[::-step_stride, :, 2].T)
    for idx,line in enumerate(ax.lines): line.set_color((cs[idx], 0.5, 0.5))
    ax.plot(xs, tajectory[0, :, 2], '--', color='black')
    ax.plot(xs, tajectory[-1, :, 2], color='black')

    ax = fig.add_subplot(414)
    ax.plot(xs, tajectory[::-step_stride, :, 3].T)
    for idx,line in enumerate(ax.lines): line.set_color((cs[idx], 0.5, 0.5))
    ax.plot(xs, tajectory[0, :, 3], '--', color='black')
    ax.plot(xs, tajectory[-1, :, 3], color='black')
